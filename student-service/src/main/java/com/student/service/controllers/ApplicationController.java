...
public class ApplicationController {
	...
	public ResponseEntity<Object> lesEtudiants() {
		...
		try {
			...
			etudiantRepository.findAll().forEach((t)->{
				if (t.getFiliaire() != null) {
					Filiaire f = new Filiaire(t.getFiliaire().getDesignation(),t.getFiliaire().getCode());
					f.setEtudiants(null);
					t.setFiliaire(f);
				} else {
					t.setFiliaire(null);
	...
	public ResponseEntity<Object> creerEtudiant(@Valid @RequestBody EtudiantDto etudiantDto, BindingResult result) {
		...
		try {
			 ...
			 */
			Filiaire filiaire = filiaireRepository.findByCode(etudiantDto.getFiliaire_id());
			...
			if (etudiantDto.getId() != null)
				etudiant.setId(Integer.parseInt(etudiantDto.getId()));
			//ticket.setCreatedAt(new Date());
			//ticket.setStatus("Pending");
			//System.out.println(ticket.getStatus());
			etudiant.setNumINE(UUID.randomUUID().toString());
			etudiant.setFiliaire(filiaire);
	...
	//@CrossOrigin(origins = "*")
	public ResponseEntity<Object> creerFiliaire(@Valid @RequestBody Filiaire filiaireDto, BindingResult result) {
		...
		try {
			...
			//ticket.setStatus("Pending");
			//System.out.println(ticket.getStatus());
			filiaireRepository.save(filiaireDto);
			//ticket.setCustomer(null);
			/*
			 * customer.getWallets().forEach((w)->{ w.setWalletType(new
			 * WalletType(w.getWalletType().getWtpId(),w.getWalletType().getWtpLabel(),w.
			...
			var response = new HashMap<String, Object>();
			response.put("filiaire", filiaireDto);
			//System.out.println(ticket);
